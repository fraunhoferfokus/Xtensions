...
public final class OptionalExtensions {
	...
	public static void forEachInt(@NonNull Optional<? extends IntegerRange> self, @NonNull IntConsumer consumer) {
		if (self.isPresent()) {
			IntegerRangeExtensions.forEachInt(self.get(), consumer);
		}
	}
	 ...
	 * @param consumer
	 */
	public static <T> void forEach(@NonNull Optional<? extends Iterable<T>> optRange,
			@NonNull Consumer<? super T> consumer) {
		if (optRange.isPresent()) {
			optRange.get().forEach(consumer);
		}
	}

	public static <T> void forEach(@NonNull Optional<? extends Iterable<T>> optRange,
			@NonNull ObjLongConsumer<T> consumer) {
		if (optRange.isPresent()) {
			final Iterator<T> it = optRange.get().iterator();
			long index = 0;
			while (it.hasNext()) {
	...
	/**
	 * Shortcut for {@code opts.map(Arrays::asList)}.
	 * 
	 * @param opts
	 * @return
	 */
	@Pure
	public static <T> @NonNull Optional<List<T>> asList(@NonNull Optional<T[]> opts) {
		return opts.map(Arrays::asList);
	}
	...
	public static void forEach(@NonNull Optional<int[]> optInts, @NonNull IntConsumer consumer) {
		if (optInts.isPresent()) {
			final int[] nums = optInts.get();
			for (int i = 0; i < nums.length; i++) {
				consumer.accept(nums[i]);
			}
		}
	}

	public static void forEach(@NonNull Optional<int[]> optInts, @NonNull IntIntConsumer consumer) {
		if (optInts.isPresent()) {
			final int[] nums = optInts.get();
			for (int i = 0; i < nums.length; i++) {
	...
	public static void forEach(@NonNull Optional<long[]> optInts, @NonNull LongConsumer consumer) {
		if (optInts.isPresent()) {
			final long[] nums = optInts.get();
			for (int i = 0; i < nums.length; i++) {
				consumer.accept(nums[i]);
			}
		}
	...
	public static void forEach(@NonNull Optional<int[]> optInts, @NonNull LongIntConsumer consumer) {
		if (optInts.isPresent()) {
			final int[] nums = optInts.get();
			for (int i = 0; i < nums.length; i++) {
				consumer.accept(nums[i], i);
	...
	public static void forEach(@NonNull Optional<double[]> optDouble, @NonNull DoubleConsumer consumer) {
		if (optDouble.isPresent()) {
			final double[] nums = optDouble.get();
			for (int i = 0; i < nums.length; i++) {
				consumer.accept(nums[i]);
			}
		}
	...
	public static void forEach(@NonNull Optional<double[]> optDouble, @NonNull DoubleIntConsumer consumer) {
		if (optDouble.isPresent()) {
			final double[] nums = optDouble.get();
			for (int i = 0; i < nums.length; i++) {
				consumer.accept(nums[i], i);
