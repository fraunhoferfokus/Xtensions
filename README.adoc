...
written behind the parenthesis. Have a look at the section <<Async Computations>> for a more Xtend style API.

INFO: Currently there there are no `thenAsync` versions of the `then` methods implemented, but they are 
...
the returned future will be completed exceptionally with the exception thrown by the callback.
...
////
Problems with CompletableFuture supplyAsync: Executor wrong arg, cancellation info
