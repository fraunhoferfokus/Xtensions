...
When comparing Xtend's `Function1` type (and their extensions in 
http://download.eclipse.org/modeling/tmf/xtext/javadoc/2.9/org/eclipse/xtext/xbase/lib/FunctionExtensions.html[`org.eclipse.xtext.xbase.lib.FunctionExtensions`])
to Java 8's `java.util.function.Function` type the method `andThen` from Java 8 is missing, so the same functionality is provided
in this librarie's `FunctionExtensions`
